...
public class tree_distence {
	...
	public int treedist(tree t1, tree t2, int i, int j) {
		...
		for(int i1 = t1.GetMostLeftNode(i); i1 <= i; i1++) {
			for(int j1 = t2.GetMostLeftNode(j); j1 <= j; j1++) {
				 int i_temp = (t1.GetMostLeftNode(i) > i1 - 1) ? 0 : i1 - 1;
		         int j_temp = (t2.GetMostLeftNode(j) > j1 - 1) ? 0 : j1 - 1;
				...
				if((t1.GetMostLeftNode(i1) == t1.GetMostLeftNode(i)) && (t2.GetMostLeftNode(j1) == t2.GetMostLeftNode(j))){
					...
					System.out.println("forestdist"+i_temp+","+j_temp+"="+forestdist[i_temp][j_temp]+";"+"tree_dist"+i1+","+j1+"="+tree_dist[i1][j1]);
				}
				 else{
		             ...
		             int i_temp2 = (t1.GetMostLeftNode(i) > i1_temp) ? 0 : i1_temp;
		             int j_temp2 = (t2.GetMostLeftNode(j) > j1_temp) ? 0 : j1_temp;
